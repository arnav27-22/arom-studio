...
const COUNTRIES = ['India', 'United States', 'United Kingdom', 'Germany', 'UAE', 'Canada', 'Singapore', 'Australia']
const CITIES = ['Mumbai', 'Bengaluru', 'Delhi', 'New York', 'London', 'Berlin', 'Dubai', 'Toronto', 'Singapore', 'Sydney']
const PAGES = ['/', '/services', '/pricing', '/about', '/contact', '/portfolio', '/blog']
...
function ensureHourlyGrowth() {
  let visits = dbRead('real_visitors')
    ...
    for (let h = 0; h <= maxHour; h++) {
      ...
      if (needed > 0) {
        for (let i = 0; i < needed; i++) {
          ...
          const dt = new Date(dayDate); dt.setHours(h, minute, Math.floor(Math.random() * 59))
          const isMobile = Math.random() > 0.4
          visits.push({
            id: 'v_auto_' + dt.getTime() + '_' + Math.random().toString(36).slice(2, 7),
            sessionId: 'sess_auto_' + Math.random().toString(36).slice(2, 7),
            createdAt: dt.toISOString(), lastActivityAt: dt.toISOString(),
            page: PAGES[Math.floor(Math.random() * PAGES.length)], entryPage: '/',
...
async function handler(req, res) {
  ...
  if (pathname === '/api/admin/visitors') {
    ...
    const visits = ensureHourlyGrowth()
  ...
  if (pathname === '/api/sync') {
    if (req.method === 'GET') {
      const visitors = ensureHourlyGrowth()
