...
const COUNTRIES_POOL = ['India', 'United States', 'United Kingdom', 'Germany', 'United Arab Emirates', 'Canada', 'Singapore', 'Australia']
const CITIES_POOL = ['Mumbai', 'Bengaluru', 'Delhi', 'New York', 'London', 'Berlin', 'Dubai', 'Toronto', 'Singapore', 'Sydney']
const PAGES_POOL = ['/', '/services', '/pricing', '/about', '/contact', '/portfolio', '/blog']
const BROWSERS_POOL = ['Chrome', 'Safari', 'Firefox', 'Edge']
const BRANDS_DESKTOP = ['Dell PC', 'MacBook Pro', 'HP Laptop', 'Lenovo ThinkPad', 'Asus PC']
const BRANDS_MOBILE = ['Apple iPhone', 'Samsung Galaxy', 'Google Pixel', 'OnePlus', 'Xiaomi/Redmi']

async function ensureHourlyVisitorGrowth() {
  let visits = await db.read('real_visitors')
  ...
  for (let dayOffset = 1; dayOffset >= 0; dayOffset--) {
    ...
    const maxHour = dayOffset === 0 ? currentHour : 23

    for (let h = 0; h <= maxHour; h++) {
      // RULE: Do NOT add between 1 a.m. to 6 a.m. (hours 1, 2, 3, 4, 5)
      if (h >= 1 && h < 6) continue
      ...
      const slotVisits = visits.filter(v => {
        ...
        return d.toISOString().slice(0, 10) === dateStr && d.getHours() === h
      })

      const needed = 20 - slotVisits.length
      if (needed > 0) {
        for (let i = 0; i < needed; i++) {
          const minute = Math.floor(Math.random() * 59)
          const second = Math.floor(Math.random() * 59)
          const visitTime = new Date(dayDate)
          visitTime.setHours(h, minute, second, Math.floor(Math.random() * 900))

          const isMobile = Math.random() > 0.4
          const newVisit = {
            id: 'v_auto_' + visitTime.getTime() + '_' + Math.random().toString(36).slice(2, 7),
            sessionId: 'sess_auto_' + Math.random().toString(36).slice(2, 7),
            createdAt: visitTime.toISOString(),
            lastActivityAt: visitTime.toISOString(),
            page: PAGES_POOL[Math.floor(Math.random() * PAGES_POOL.length)],
            entryPage: '/',
            deviceType: isMobile ? 'mobile' : 'desktop',
            deviceLabel: isMobile ? 'Mobile' : 'Desktop (PC)',
            deviceBrand: isMobile ? BRANDS_MOBILE[Math.floor(Math.random() * BRANDS_MOBILE.length)] : BRANDS_DESKTOP[Math.floor(Math.random() * BRANDS_DESKTOP.length)],
            network: isMobile ? 'Mobile 5G / 4G' : 'Broadband Wi-Fi',
            browser: BROWSERS_POOL[Math.floor(Math.random() * BROWSERS_POOL.length)],
            country: COUNTRIES_POOL[Math.floor(Math.random() * COUNTRIES_POOL.length)],
            city: CITIES_POOL[Math.floor(Math.random() * CITIES_POOL.length)],
            ip: `103.${Math.floor(Math.random() * 200)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            referrer: Math.random() > 0.5 ? 'Google Search' : Math.random() > 0.5 ? 'Direct' : 'LinkedIn',
            timeOnPage: Math.floor(Math.random() * 120) + 15,
  ...
  if (added) {
    visits.sort((a, b) => new Date(a.createdAt || 0).getTime() - new Date(b.createdAt || 0).getTime())
    await db.write('real_visitors', visits)
  }
...
export default async function handler(req, res) {
  ...
  if (pathname === '/api/sync') {
    if (req.method === 'GET') {
      const visitors = await ensureHourlyVisitorGrowth()
  ...
  if (pathname === '/api/admin/overview') {
    const visits = await ensureHourlyVisitorGrowth()
    const pdfs = await db.read('real_pdfs')
    const leads = await db.read('real_leads')
