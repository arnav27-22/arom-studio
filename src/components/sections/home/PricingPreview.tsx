    ...
    <Section>
      <Container>
            ...
            <FadeIn key={plan.name} delay={i * 0.1}>
              ...
              >
                ...
                <div className="mb-5">
                  ...
                  <p className="text-xs text-white/75 font-body mt-2">{plan.description}</p>
                </div>

                <ul className="space-y-2.5 flex-1 mb-6">
                  {plan.features.slice(0, 5).map((f) => (
                    <li key={f} className="flex items-start gap-2 text-sm text-white/80 font-body font-light">
                      <Check className="h-4 w-4 text-accent shrink-0 mt-0.5" />
                      {f}
                    </li>
                  ))}
                  {plan.features.length > 5 && (
                    <li className="text-xs text-white/70 font-body pl-6">+{plan.features.length - 5} more features</li>
                  )}
                </ul>
        ...
        <FadeIn delay={0.3} className="text-center mt-10">
          <p className="text-sm text-white/75 font-body mb-4">Need a custom solution? Let&apos;s discuss your project.</p>
          <Link
            to="/contact"
