...
import { createDoc, getPageLayout, addCoverPage, addHeader, writeSection, checkPageBreak, finalizeDoc, applyContentPageHeaders, today, type PageLayout } from './professionalPDF'
...
export function buildAgreementPDF(data: AgreementData): jsPDF {
  ...
  const advPct = data.advancePercentage || '50'
  const finPct = data.finalPercentage || '50'
  ...
  y = writeSection(doc, y, '1. Parties', [
    ...
    `The Client's name is ${data.clientName || '[Client Name]'}${data.clientAddress ? `, with address at ${data.clientAddress}` : ''}${data.clientEmail ? `, and contact email ${data.clientEmail}` : ''}${data.clientPhone ? `, phone ${data.clientPhone}` : ''}.`,
  ...
  const servicesText = data.selectedServices && data.selectedServices.length > 0
    ? data.selectedServices.join(', ')
    : 'website design, website development, responsive design, landing page creation, e-commerce functionality, custom feature development, CMS integration, SEO optimization, and website deployment'
  y = writeSection(doc, y, '4. Scope of Work', [
    `The Agency agrees to provide the Services as defined in the Project Proposal, which may include ${servicesText}.`,
  ...
  y = writeSection(doc, y, '9. Payment Terms', [
    `The Client agrees to pay the Agency the total Project fee as set forth in the Proposal. An advance payment of ${advPct}% of the total fee shall be due before work commences, and the remaining ${finPct}% shall be due prior to final delivery or deployment of the Website.`,
  ...
  // Section 19: Warranty
  const warrantyDays = data.supportPeriod || '30'
  y = writeSection(doc, y, '19. Warranty', [
    `The Agency warrants that the Deliverables will conform to the specifications set forth in the Proposal and will be free from material defects in coding and functionality for a period of ${warrantyDays} days from the date of delivery or deployment (the "Warranty Period").`,
  ...
  // Section 29: Security Disclaimer
  ...
  // Section 30: Electronic Signatures
...
function writeContactFooter(doc: jsPDF, y: number, layout: PageLayout): number {
  ...
  doc.text('AROM STUDIO  |  aromstudio27@gmail.com  |  +91 8767990061  |  https://arom-studio.vercel.app', layout.marginLeft, y)
  y += 10
